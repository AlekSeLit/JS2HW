<template>
  <div :class="[$style.wrapper]">
    <Button @mySuperEvent="onCartClick">
      <svg
    ...
    <div :class="[$style.list]" v-if="opened">
      ...
      <div v-for="(id, index) in getItemsInCart" :key="id + `${index}`">
        <div :class="[$style.basket]">
          <p :class="[$style.basket_name]">Name</p>
          <p>Price</p>
        </div>
        <div :class="[$style.basket]">
          <p :class="[$style.basket_name]">{{ getData[id].name }}</p>
          <p>{{ getData[id].price }}</p>
        </div>
      </div>
    </div>
  </div>
